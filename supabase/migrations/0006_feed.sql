-- Feed ranking. §21 + Appendix B.
-- A transparent, tunable scoring function with weights read from app_config, a
-- mandatory post-ranking diversity pass, reserved exploration slots, and a
-- fallback chain so the deck is never empty while any publishable recipe exists.

-- Card payload for the deck (§8.1). Deliberately narrow: anything that does not
-- change whether someone would cook the dish belongs on the detail page.
create or replace function public.recipe_card(p_recipe_id uuid)
returns jsonb language sql stable security definer set search_path = public as $$
  select jsonb_build_object(
    ...
    'isSponsored', r.sponsorship_id is not null,
    'creator', jsonb_build_object(
      'id', u.id, 'username', u.username, 'displayName', u.display_name,
      'avatarUrl', u.avatar_url, 'isSeedAccount', u.is_seed_account),
    'tags', coalesce((
      select jsonb_agg(jsonb_build_object('slug', t.slug, 'name', t.name, 'type', t.type)
                       order by t.type, t.name)
      from recipe_tags rt join tags t on t.id = rt.tag_id
      where rt.recipe_id = r.id), '[]'::jsonb),
    'saveCount', coalesce(s.save_count, 0),
    'cookCount', coalesce(s.cook_count, 0)
  )
  from recipes r
...
-- Note: the strict-category-gate behaviour above (v_strict) was applied to the
-- live project as a follow-up migration named `feed_strict_category_gates`.
-- It is folded into this file, so replaying 0001-0010 from scratch produces the
-- same final state.
